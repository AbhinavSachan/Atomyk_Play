...
object MusicHelper {
    ...
    fun convertDurationForLyrics(duration: String): String {
        return formatDuration(duration.toInt(), includeMillis = true)
    }
    ...
    fun convertDuration(duration: String): String {
        return formatDuration(duration.toInt(), includeMillis = false)
    }

    private fun formatDuration(duration: Int, includeMillis: Boolean): String {
        ...
        return if (hours == 0) {
            if (includeMillis) {
                String.format(Locale.ENGLISH,"%02d:%02d.%02d", minutes, seconds, milliseconds / 10)
            } else {
                String.format(Locale.ENGLISH,"%02d:%02d", minutes, seconds)
            }
        } else {
            if (includeMillis) {
                String.format(Locale.ENGLISH,"%02d:%02d:%02d.%02d", hours, minutes, seconds, milliseconds / 10)
            } else {
